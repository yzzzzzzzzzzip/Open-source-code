...
#include "at24.h"
/******************************* 全局配置 ***************************/
#define UART_TIMEOUT_MS         5000        // 串口阻塞超时时间（单位：ms）
#define RX_BUF_MAX_LEN          2048        // 串口1最大接收缓存字节数（存储ESP8266响应数据）
...
#define MQTT_CLIENT_ID         "RB"  											// MQTT 客户端 设备名称
#define MQTT_USER_NAME         "VAvg8087hx"      					// MQTT 服务器产品id
...
#define HTTP_PASSWORD          "version=2022-05-01&res=userid%2F488021&et=8080603082&method=md5&sign=Ni%2BtWD%2FCflk3%2BvtP040qAA%3D%3D"
#define MQTT_cert_key_ID        0                        	// MQTT cert 证书, 参数为 0
#define MQTT_CA_ID							0													// MQTT目前支持一套 CA 证书, 参数为 0
#define MQTT_CONNECT_TIMEOUT   2000                      	// MQTT 连接超时（ms，安可信固件耗时略长）
...
#define TCP_SERVER_IP          "iot-api.heclouds.com" // 远端TCP服务器IP地址（需与ESP8266在同一网段）
#define TCP_SERVER_PORT        80            // 远端TCP服务器端口号（自定义，需与服务器一致）

// 新增：OTA接收模式枚举（放在头文件）
typedef enum {
    OTA_RECV_MODE_NONE,        // 默认(固件下载）
    OTA_RECV_MODE_CHECK_VER,   // 版本检查（JSON响应，结束标志：JSON末尾}）
    OTA_RECV_MODE_DOWNLOAD     // 固件下载（二进制响应，结束标志：\r\n\r\n+指定长度）
} OTA_Recv_ModeTypeDef;

// HTTP请求常量定义
#define OTA_HTTP_GET_REQUEST_FMT \
...
// OTA版本检查请求格式宏（对应你提供的GET请求）
#define OTA_CHECK_VERSION_REQUEST_FMT \
    "GET /fuse-ota/%s/%s/check?type=1&version=1.2 HTTP/1.1\r\n" \
    "Content-Type:application/json\r\n" \
    "Authorization:%s\r\n" \
    "host:%s\r\n" \
    "\r\n" \
	"\r\n"
// JSON解析相关宏（便于提取字段）
#define JSON_FIELD_TARGET    "\"target\":\"%[^\"]\""   // 匹配"target":""
#define JSON_FIELD_SIZE      "\"size\":%d"             // 匹配"size":
#define JSON_FIELD_MD5       "\"md5\":\"%[^\"]\""      // 匹配"md5":""
#define JSON_FIELD_TID       "\"tid\":%d"              // 匹配"tid":

// 固件信息结构体（存储解析后的结果）
typedef struct {
    char current_version[16];  // 新增：当前版本（如"1.0"）
    char target_version[16];   // 目标版本（如"1.4"）
    uint32_t tid;              // 固件ID（如1310012）
    uint32_t size;             // 固件总大小（如7360）
    char md5[33];              // MD5值（32位+结束符）
    uint8_t code;              // 响应码（0=成功）
    uint8_t parse_ok;          // 解析成功标志（1=成功，0=失败）
	uint32_t crc32;
} OTA_Firmware_InfoTypeDef;
...
extern OTA_Firmware_InfoTypeDef g_ota_firmware_info;
...
/**
 * @brief  打印ESP8266接收的OTA固件数据（ASCII头+二进制数据）
 * @note   1. 自动识别HTTP头和二进制数据分界
 *         2. ASCII头打印字符，二进制数据打印十六进制
 *         3. 基于你封装的UART2_Debug_Print实现
 */
